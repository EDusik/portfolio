	...
	t && Object.deepExtend(m, t),
		...
		(m.fn.particle = function (e, t, a) {
			...
			m.particles.move.straight
				? ((this.vx = s.x),
				  (this.vy = s.y),
				  m.particles.move.random && ((this.vx = this.vx * Math.random()), (this.vy = this.vy * Math.random())))
			...
			if ("image" == this.shape) {
				...
				(this.img = { src: c.image.src, ratio: c.image.width / c.image.height }),
					...
					"svg" == m.tmp.img_type &&
						...
						(m.fn.vendors.createSvgImg(this), m.tmp.pushing && (this.img.loaded = !1));
			}
		}),
		...
		(m.fn.particlesUpdate = function () {
			for (var e = 0; e < m.particles.array.length; e++) {
				...
				if (
					...
					i = { x_left: -t.radius, x_right: m.canvas.w + t.radius, y_top: -t.radius, y_bottom: m.canvas.h + t.radius };
				switch (
					(t.x - t.radius > m.canvas.w
						? ((t.x = i.x_left), (t.y = Math.random() * m.canvas.h))
						: t.x + t.radius < 0 && ((t.x = i.x_right), (t.y = Math.random() * m.canvas.h)),
					t.y - t.radius > m.canvas.h
						? ((t.y = i.y_top), (t.x = Math.random() * m.canvas.w))
						: t.y + t.radius < 0 && ((t.y = i.y_bottom), (t.x = Math.random() * m.canvas.w)),
		...
		(m.fn.interact.linkParticles = function (e, t) {
			...
			if (s <= m.particles.line_linked.distance) {
				var n =
					...
					s / (1 / m.particles.line_linked.opacity) / m.particles.line_linked.distance;
				if (0 < n) {
				...
				function v(e, t, a, i, s) {
					if (e != t)
						if (m.tmp.bubble_duration_end)
							null != a &&
								((r = e + (e - (i - (l * (i - e)) / m.interactivity.modes.bubble.duration))),
		...
		(m.fn.modes.repulseParticle = function (i) {
			if (
				...
				var e = i.x - m.interactivity.mouse.pos_x,
					...
					n = t / a,
					r = clamp(
						(1 / (o = m.interactivity.modes.repulse.distance)) * (-1 * Math.pow(a / o, 2) + 1) * o * 100,
						0,
						50
					),
				...
				if (
					(m.tmp.repulse_finish ||
						(m.tmp.repulse_count++, m.tmp.repulse_count == m.particles.array.length && (m.tmp.repulse_finish = !0)),
			...
			if (m.interactivity.events.onhover.enable && "mousemove" == m.interactivity.status) {
				...
				if (i <= m.interactivity.modes.grab.distance) {
					var s =
						...
						i / (1 / m.interactivity.modes.grab.line_linked.opacity) / m.interactivity.modes.grab.distance;
					if (0 < s) {
				...
				(m.interactivity.events.onhover.enable || m.interactivity.events.onclick.enable) &&
					(m.interactivity.el.addEventListener("mousemove", function (e) {
						...
						(m.interactivity.mouse.pos_x = t),
							...
							m.tmp.retina &&
								((m.interactivity.mouse.pos_x *= m.canvas.pxratio), (m.interactivity.mouse.pos_y *= m.canvas.pxratio)),
							...
							switch (m.interactivity.events.onclick.mode) {
								case "push":
									m.particles.move.enable
										...
										? m.fn.modes.pushParticles(m.interactivity.modes.push.particles_nb, m.interactivity.mouse)
										: 1 < m.interactivity.modes.push.particles_nb &&
										  m.fn.modes.pushParticles(m.interactivity.modes.push.particles_nb);
									break;
								case "remove":
		...
		(m.fn.vendors.start = function () {
			isInArray("image", m.particles.shape.type)
				? ((m.tmp.img_type = m.particles.shape.image.src.substr(m.particles.shape.image.src.length - 3)),
